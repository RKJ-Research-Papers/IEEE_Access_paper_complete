...
\author{\uppercase{Ravi Kumar Jain}\authorrefmark{1},
and \uppercase{Anju Goswami}\authorrefmark{1}}

\address[1]{Indian Institute of Foreign Trade (IIFT), Delhi, India (e-mail: ravi\_phdm23@iift.edu, anju@iift.edu)}

\tfootnote{This work was submitted for review to IEEE Access.}

\markboth
{Jain and Goswami: Beyond Attribution: A Falsifiability Framework for Reliable Credit Risk Explanations}
{Jain and Goswami: Beyond Attribution: A Falsifiability Framework for Reliable Credit Risk Explanations}

\corresp{Corresponding authors: Ravi Kumar Jain (e-mail: ravi\_phdm23@iift.edu); Anju Goswami (e-mail: anju@iift.edu).}
...
\midrule
Processing &
Sorts features by absolute SHAP magnitude; selects top validated features \\
\midrule
Prompt Structure &
System prompt constrains LLM role as ML analyst; user prompt provides context; output: two paragraphs on (1) prediction reasoning, (2) prediction-label alignment \\
\midrule
Output &
Two paragraphs: SHAP-driven reasoning and correctness \\
\midrule
Critical Constraint &
Narrative grounded in validated SHAP evidence; no unsupported claims; suppressed if signal $<$ noise \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.75\textwidth]{architecture_diagram_tikz.pdf}
...
\begin{tabular}{p{0.9cm}p{1.4cm}p{2.2cm}p{3.8cm}}
\toprule
\textbf{Metric} & \textbf{Category} & \textbf{Purpose} & \textbf{Formula} \\
\midrule
AUC & Discrimin. & Rank-ordering & $\int_0^1 \mathrm{TPR}\, d\mathrm{FPR}$ \\
KS & Discrimin. & Max separation & $\max_{t} |\mathrm{TPR}(t) - \mathrm{FPR}(t)|$ \\
\midrule
BS & Calibration & Prob. error & $\frac{1}{N} \sum_{i=1}^N (p_i - y_i)^2$ \\
\midrule
PCC & Classif. & Accuracy & $\frac{\mathrm{TP} + \mathrm{TN}}{\mathrm{TP} + \mathrm{FP} + \mathrm{TN} + \mathrm{FN}}$ \\
Recall & Classif. & Sensitivity & $\frac{\mathrm{TP}}{\mathrm{TP} + \mathrm{FN}}$ \\
H & Utility & Cost-sensitive \cite{Hand2009_hmeasure} & $1 - \frac{\mathrm{EMC}}{\mathrm{EMC}_0}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\caption{Canonical Model Instantiations: Performance Metrics}
...
% AUTHOR BIOGRAPHIES
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{C:/Users/Arnav/OneDrive/Desktop/Publishing/IEEE Access paper/Author details/RKJ.png}}]{Ravi Kumar Jain}
is a Research Scholar at the Indian Institute of Foreign Trade (IIFT), Delhi, India. His research interests include Credit Risk Modeling, Artificial Intelligence in Finance, Explainable AI, and Machine Learning Governance.
\end{IEEEbiography}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{Anju_maam.jpg}}]{Dr. Anju Goswami}
received the doctoral degree in Finance and Decision Sciences from the Indian Institute of Foreign Trade (IIFT), New Delhi, India, where she is currently a faculty member. Her academic and research work focuses on Financial Risk Management, Credit Risk Assessment, and the application of Data-Driven and Artificial Intelligence–Based methods in Banking and Financial Systems.

She has been involved in postgraduate teaching and research in Finance and Analytics and has contributed to peer-reviewed journal publications and academic conferences. Her current research interests include Credit Risk Modeling, Machine Learning Applications in Finance, Regulatory Risk Frameworks, and Decision Support Systems for Financial Institutions.
\end{IEEEbiography}
